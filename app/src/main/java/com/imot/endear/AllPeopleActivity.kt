...
class AllPeopleActivity : AppCompatActivity(), IfirebaseLoadDone {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recycler_all_user.addItemDecoration(DividerItemDecoration(this,layoutManager.orientation))
                    ...
                    .addListenerForSingleValueEvent(object:ValueEventListener{
                            ...
                            if (snapshot.value == null)//not yet friend
                        {
                             sendFriendRequest(model)
                                ...
                                Toast.makeText(this@AllPeopleActivity, model.email+"est déjà présent dans la liste de vos proches.",Toast.LENGTH_SHORT).show()
                            }
                        }
        ...
        tokens.orderByKey().equalTo(model.uid)
            .addListenerForSingleValueEvent(object : ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    {
                       Toast.makeText(this@AllPeopleActivity,"La personne que vous " +
                               "esssayez de joindre n'est pas disponible.", Toast.LENGTH_SHORT).show()
                    }else{
                        //Create request
                        val request = Request()
